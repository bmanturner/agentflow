...
pub async fn run_smoke_test(omp: &str, repo_root: &Path) -> Result<()> {
    ...
    rpc.prompt_and_wait(&format!(
        "Remember {token}. Reply with exactly: remembered {token}."
    ))
    .await?;
    rpc.prompt_and_wait("What token did I ask you to remember? Reply with only the token.")
        .await?;
    ...
    rpc.prompt_and_wait("What token did I ask you to remember? Reply briefly.")
        .await?;
